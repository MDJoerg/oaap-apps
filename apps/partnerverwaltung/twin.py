...
def create_object(type_key, title, group_key, attributes=None, relations=None,
                   activities=None, source_key=None, attr_valid_from=None,
                   attr_valid_to=None):
    """POST /twin/objects (oaap.data.twin 0.1 SS2.5) -- refused unless the
    caller owns 'type_key'. 'attributes' applies attr_valid_from/_to to
    EVERY attribute in this one call (the service reads it at group-
    payload level); each entry of 'relations' carries its own
    valid_from/valid_to instead, because the service reads THOSE per
    relation, not from the group payload -- the two loops in
    services/twin/app.py's _write_group_content are not symmetric, and
    this client follows that exactly rather than a tidier guess."""
    group = {"key": group_key}
    if attributes:
        group["attributes"] = attributes
        if attr_valid_from:
            group["valid_from"] = attr_valid_from
        if attr_valid_to:
            group["valid_to"] = attr_valid_to
    if relations:
        group["relations"] = relations
    if activities:
        group["activities"] = activities
    body = {"type": type_key, "title": title, "group": group}
    if source_key:
        body["source_key"] = source_key
    return _call("POST", "/twin/objects", body)
